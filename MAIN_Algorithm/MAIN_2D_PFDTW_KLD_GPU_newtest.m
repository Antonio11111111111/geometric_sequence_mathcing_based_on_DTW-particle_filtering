%==========================================================================
% SCRIPT: MAIN_2D_PFDTW_KLD_GPU (Path Constraints)
%==========================================================================
clc;
clear;
close all;
...
function dist = dtw_sakoe_chiba(seq1, seq2, r)
    % 获取序列长度
    n = length(seq1);
    
    % 初始化累积代价矩阵，全部填满正无穷大
    D = inf(n+1, n+1, 'single');
    D(1,1) = 0;
    
    % 只在允许的 Sakoe-Chiba 窗口带内进行动态规划计算
    for i = 1:n
        j_start = max(1, i - r);
        j_end = min(n, i + r);
        for j = j_start:j_end
...
function [particles_out_gpu, best_guess_gpu, d_min_gpu] = Particle_Filter_DTW_Step_2D_CPFTest(particles_in_gpu, live_sequence_gpu, ...
    ...
    parfor m = 1:M
        ...
        % 这里替换为了我们刚刚写好的带约束的 DTW 函数，窗口半径设为 3
        distance = dtw_sakoe_chiba(live_sequence_cpu, map_seq, 3);
